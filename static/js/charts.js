...
function buildCharts(sample) {
  ...
  d3.json("static/js/samples.json").then((data) => {
    ...
    var id = result.otu_ids.slice(0,10).reverse();
    var label = result.otu_labels.slice(0,10).reverse();
    var value = result.sample_values.slice(0,10).reverse();
    console.log(result);
    ...
    var yticks = id.map(sampleObj => "OTU " +  sampleObj);
    console.log(yticks);
...
// BUBBLE CHART
  ...
  Plotly.newPlot("bubble-plot", bubbleData, bubbleLayout)
   ...
   var gaugeArray = metadata.filter(sampleObj => sampleObj.id == sample);
     ...
     var wash_frequency = gaugeResult.wfreq
